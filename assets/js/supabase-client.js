...
const Supabase = {
  ...
  async insert(table, data) {
    if (window.ADMIN_API) {
      const bulkQuery = window.BULK_IMPORT_MODE ? '&bulk=1' : '';
      const res = await fetch(`/api/admin?table=${encodeURIComponent(table)}&action=insert${bulkQuery}`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
  ...
  async update(table, id, data) {
    if (window.ADMIN_API) {
      const bulkQuery = window.BULK_IMPORT_MODE ? '&bulk=1' : '';
      const res = await fetch(`/api/admin?table=${encodeURIComponent(table)}&action=update&id=${encodeURIComponent(id)}${bulkQuery}`, { method: 'PATCH', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
