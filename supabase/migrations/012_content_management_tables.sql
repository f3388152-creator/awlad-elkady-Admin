...
alter table public.faqs add column if not exists is_visible boolean not null default true;
alter table public.faqs add column if not exists sort_order integer not null default 1;
...
create policy "public read visible faqs" on public.faqs for select using (is_visible = true);
drop policy if exists "public read visible socials" on public.socials;
create policy "public read visible socials" on public.socials for select using (is_visible = true);
