...
async function isActiveStaff(user) {
  ...
  try {
    const response = await fetch(`${SUPABASE_URL}/rest/v1/staff_accounts?select=id,auth_user_id,is_active,session_version&id=eq.${encodeURIComponent(String(user.app_metadata.staff_id))}&limit=1`, {
      headers: { apikey: SERVICE_ROLE_KEY, Authorization: `Bearer ${SERVICE_ROLE_KEY}` }
    });
    if (!response.ok) return false;
    const rows = await response.json();
    const row = rows?.[0];
    return Boolean(row && row.is_active !== false && String(row.auth_user_id) === String(user.id) && Number(row.session_version || 1) === Number(user.app_metadata?.session_version || 1));
  } catch (_) {
    return false;
  }
}
