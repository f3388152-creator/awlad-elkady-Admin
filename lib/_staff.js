...
async function createStaff({ phone, display_name, password, permissions, avatar_url = null, session_enabled = false, session_minutes = 43200 }) {
  ...
  try {
    ...
    if (row?.id) {
      await authAdmin(`/${encodeURIComponent(user.id)}`, 'PUT', {
        app_metadata: { role: 'staff', permissions: permissions || {}, staff_id: row.id, session_version: Number(row.session_version || 1) },
...
async function updateStaff(id, patch) {
  ...
  if (patch.avatar_url !== undefined) next.avatar_url = patch.avatar_url || null;
  ...
  if (patch.permissions !== undefined || patch.display_name !== undefined) {
    authPatch.app_metadata = { role: 'staff', permissions: next.permissions || current.permissions || {}, staff_id: id, session_version: Number(current.session_version || 1) };
...
async function forceLogoutStaff(id) {
  ...
  await supabase(`/rest/v1/staff_accounts?id=eq.${encodeURIComponent(id)}`, 'PATCH', { session_version: nextVersion, is_online: false, last_seen_at: new Date().toISOString() }, 'return=minimal');
  try {
    await authAdmin(`/${encodeURIComponent(current.auth_user_id)}`, 'PUT', {
      app_metadata: { role: 'staff', permissions: current.permissions || {}, staff_id: id, session_version: nextVersion },
      ban_duration: 'none'
    });
  } catch (_) { /* database session version remains the source of truth */ }
  return { ok: true, session_version: nextVersion };
}

module.exports = { normalizePhone, validPhone, loginEmail, cleanStaff, findByPhone, findById, listStaff, createStaff, updateStaff, deleteStaff, forceLogoutStaff };
